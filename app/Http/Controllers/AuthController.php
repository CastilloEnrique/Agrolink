...
//    }
    public function logout(Request $request)
    {
        // Obtiene el usuario autenticado (ya sabemos que existe por el middleware)
        $user = $request->user();

        // Revoca el token de API que el usuario está usando para esta petición
        $user->currentAccessToken()->delete();

        return response()->json(['message' => 'Sesión cerrada exitosamente (token revocado)']);
    }
