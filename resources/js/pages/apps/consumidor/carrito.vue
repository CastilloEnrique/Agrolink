...
const isProcessing = ref(false) // Lo mantenemos por si lo usamos en el futuro

// Cargamos el carrito al montar la página
onMounted(async () => {
  await cartStore.fetchCart()
})

// --- 💡 1. LÓGICA DE CONSOLIDAR PEDIDO ELIMINADA ---
// Ya no llamamos a la API de 'pedidos' desde aquí.

// --- 💡 2. NUEVA FUNCIÓN PARA IR AL CHECKOUT ---
const goToCheckout = () => {
  ...
  // Navegamos a la nueva página que vamos a crear
  router.push({ name: 'apps-consumidor-checkout' })
}

// Lógica para actualizar cantidad (sin cambios)
const handleQuantityChange = (id, event) => {
  ...
  // Llamamos a la acción de Pinia (que llama a la API)
      ...
      <VCard title="Resumen del Pedido">
        ...
        <VCardActions class="d-flex flex-column gap-2">
          ...
          <VBtn
            ...
            @click="goToCheckout"
          >
            Ir a Pagar y Seleccionar Entrega
          </VBtn>
          <VBtn
            block
            variant="tonal"
